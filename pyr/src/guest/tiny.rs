...
const TINY_GUEST_ENTRY_IPA: u64 = 0x4000_0000;
const TINY_GUEST_STACK_IPA: u64 = 0x4002_0000;
const TINY_GUEST_STACK_SIZE: usize = 16 * 1024;
...
pub fn load_tiny_guest() -> (IpaAddr, PhysAddr, usize) {
    ...
    crate::log!("tiny guest IPA = {:#018x}", TINY_GUEST_ENTRY_IPA);
    crate::log!("tiny guest PA  = {:#018x}", host_pa);

    (
        IpaAddr::new(TINY_GUEST_ENTRY_IPA),
        PhysAddr::new(host_pa),
        len,
    )
}

pub fn enter_tiny_guest() -> ! {
    let stack_top = TINY_GUEST_STACK_IPA + TINY_GUEST_STACK_SIZE as u64;

    enter_el1_guest(super::config::GuestConfig::new(
        TINY_GUEST_ENTRY_IPA,
        stack_top,
    ))
}
